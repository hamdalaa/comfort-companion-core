...
import { Award, ExternalLink, Heart, Loader2, Scale, ShieldCheck, Store } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { getProduct, getProductOffers, formatIQD } from "@/lib/unifiedSearch";
...
export function QuickViewDialog({ productId, open, onOpenChange, initialProduct }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      initialProduct ? Promise.resolve(initialProduct) : getProduct(productId),
...
function QuickViewBody({
  ...
  const heroImage = optimizeImageUrl(primary, { width: 720, height: 720 }) ?? primary;
    ...
    <div className="grid gap-6 p-5 sm:grid-cols-[260px_1fr] sm:gap-7 sm:p-7">
      ...
      <div className="flex min-w-0 flex-col">
        ...
        <div className="mt-4 flex items-baseline gap-3">
          <span className="font-outfit text-3xl font-semibold tabular-nums tracking-tight text-foreground">
            {product.lowestPrice ? formatIQD(product.lowestPrice) : "—"}
          </span>
          {product.highestPrice && product.highestPrice > (product.lowestPrice ?? 0) && (
            <span className="font-outfit text-sm tabular-nums text-muted-foreground/70 line-through">
