...
const prefetchProductDetail = vi.fn(() => Promise.resolve());
const prefetchProductOffers = vi.fn(() => Promise.resolve());
...
describe("prefetchRouteForPath", () => {
  ...
  it("prefetches both route code and data for product pages", async () => {
    ...
    expect(prefetchProductDetail).toHaveBeenCalledWith("unified_123");
    expect(prefetchProductOffers).toHaveBeenCalledWith("unified_123");
  });
